...
class PluginTemperatureDifferenceHotWater extends PluginAbstract
{
    ...
    public function run(PluginContext $context)
    {
        ...
        if (time() >= $timestampNextEvaluation) {
            ...
            $context->getMonitor()->set(sprintf('temperatureDifferenceHotWater%uSeconds', $this->getInterval()), sprintf('%s%u', $temperatureCurrent >= $temperatureInitial ? '+' : '-', $temperatureDifference));
